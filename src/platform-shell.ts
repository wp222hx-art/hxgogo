...
export function platformPage(html:string,key:string) {
 ...
 const nav=pages.map((p,i)=>(i===4?'<div class="hp-nav-caption">研究工具</div>':i===10?'<div class="hp-nav-caption">平台</div>':'')+`<a data-hp-route="${p[0]}" href="${p[1]}" ${key===p[0]?'aria-current="page"':''}><span aria-hidden="true">${p[2]}</span>${p[3]}</a>`).join('')
