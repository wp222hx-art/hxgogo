...
//  1) 候选只含固定 500 注策略（包含 AI），排除随机对照及所有不同注数的派生、精准档；
//  2) 排名是最近 40 期的历史描述；不足 10 期不参与，无合格候选时退回组合最优，不宣称预测优势；
...
import { STRATEGIES, META_K, ARENA_N, nextOf, loadPerf, pnlOf, rollingZ, insertRounds, verifiedLiveSql, type PerfMap } from './arena'

const r3 = (x: number) => Math.round(x * 1000) / 1000
// 同覆盖规模候选；AI 异步未到达时仅融合已经提前锁定的合格成员。
export const CANDIDATES = STRATEGIES.filter(s => !s.control && !s.derived && !s.sharp && (s.n ?? ARENA_N) === ARENA_N).map(s => s.key)   // 派生子集（ai-100 等）注数不同，不参与前三排名
...
export async function top3Round(db: D1Database, source: string, latestExpect: string) {
  const next = nextOf(latestExpect, source); if (!next) return false
  ...
  const rounds = (await db.prepare(`SELECT strategy, numbers, created_ms FROM arena_rounds WHERE source=? AND expect=? AND scored_ms IS NULL AND count=500 AND ${verifiedLiveSql()} AND cutoff_ms>${Date.now()}`).bind(source, next).all<any>()).results
  ...
  const lists = members.map(m => ({ w: m.w, numbers: (rounds.find(r => r.strategy === m.key)?.numbers || '').split(' ').filter(Boolean) })).filter(l => l.numbers.length === ARENA_N)
  if (!lists.length) return false
  const fused = lists.length === 1 ? lists[0].numbers.slice(0, ARENA_N) : fuseTop3(lists)
  const ts = Date.now()
  if (fused.length !== ARENA_N) return false
  await insertRounds(db, source, next, latestExpect, 'live', [{ strategy: TOP3_KEY, numbers: fused.map(Number), coverage: fused.length / 1000, weight: 1 }])
  await db.prepare(`INSERT OR IGNORE INTO top3_picks (source, expect, based_on, members, created_ms) VALUES (?,?,?,?,?)`).bind(source, next, latestExpect, JSON.stringify(members), ts).run()
  return true
}
...
export async function top3Backfill(db: D1Database, source: string, maxPeriods = 100) {
  ...
  for (const m of missing.reverse()) {
    const perf = await loadPerf(db, source, m.expect, 'replay')
    ...
    const lists = members.map(x => ({ w: x.w, numbers: (rounds.find(r => r.strategy === x.key)?.numbers || '').split(' ').filter(Boolean) })).filter(l => l.numbers.length === ARENA_N)
    if (!lists.length) continue
    const fused = lists.length === 1 ? lists[0].numbers.slice(0, ARENA_N) : fuseTop3(lists); const ts = Date.now()
    if (fused.length !== ARENA_N) continue
    await insertRounds(db, source, m.expect, m.based_on, 'replay', [{ strategy: TOP3_KEY, numbers: fused.map(Number), coverage: fused.length / 1000, weight: 1 }], m.actual)
    await db.prepare(`INSERT OR IGNORE INTO top3_picks (source, expect, based_on, members, created_ms) VALUES (?,?,?,?,?)`).bind(source, m.expect, m.based_on, JSON.stringify(members), ts).run()
    done++
  }
  return done
}
...
export async function top3View(db: D1Database, source: string, hist = 12, mode: 'live' | 'replay' | 'all' = 'live') {
  const auditFilter = (alias = 'arena_rounds') => mode === 'all' ? '1=1' : mode === 'replay' ? `${alias}.prediction_status='replay'` : verifiedLiveSql(alias)
  const pend = (await db.prepare(`SELECT expect, based_on, strategy, numbers, created_ms, prediction_status, prediction_version, cutoff_ms FROM arena_rounds WHERE source=? AND scored_ms IS NULL AND ${verifiedLiveSql()} AND cutoff_ms>${Date.now()} ORDER BY expect DESC LIMIT 40`).bind(source).all<any>()).results
  ...
  if (expect) {
    ...
    current = { expect, based_on: pend[0].based_on, status: fusedRow ? 'ready' : 'pending', created_ms: fusedRow?.created_ms || null, prediction_status: fusedRow?.prediction_status || null, prediction_version: fusedRow?.prediction_version || null, cutoff_ms: fusedRow?.cutoff_ms || null, members: memberRows, fused, count: fused.length, overlap, consensus_all: allThree }
  }
  // 战绩：top3 + 各候选策略总体 & 滚动
  const agg = (await db.prepare(`SELECT strategy, COUNT(*) n, SUM(hit) hits, SUM(pnl) pnl FROM arena_rounds WHERE source=? AND scored_ms IS NOT NULL AND ${auditFilter()} GROUP BY strategy`).bind(source).all<any>()).results
  ...
  const streak = (await db.prepare(`SELECT hit FROM arena_rounds WHERE source=? AND strategy=? AND scored_ms IS NOT NULL AND ${auditFilter()} ORDER BY expect DESC LIMIT 20`).bind(source, TOP3_KEY).all<any>()).results.map(r => r.hit ? 1 : 0)
  ...
  // 历史
  const rows = (await db.prepare(`SELECT a.expect, a.numbers, a.count, a.actual, a.hit, a.rank, a.pnl, a.created_ms, a.prediction_status, a.prediction_version, a.cutoff_ms, d.open_ms, p.members FROM arena_rounds a LEFT JOIN draws d ON d.source=a.source AND d.expect=a.expect LEFT JOIN top3_picks p ON p.source=a.source AND p.expect=a.expect
    WHERE a.source=? AND a.strategy=? AND a.scored_ms IS NOT NULL AND ${auditFilter('a')} ORDER BY a.expect DESC LIMIT ?`).bind(source, TOP3_KEY, hist).all<any>()).results
  ...
  const history = rows.map(r => {
    ...
    return { expect: r.expect, numbers: r.numbers, count: r.count, actual: r.actual, hit: !!r.hit, rank: r.rank, pnl: r.pnl, open_ms: r.open_ms, created_ms: r.created_ms, prediction_status: r.prediction_status, prediction_version: r.prediction_version, cutoff_ms: r.cutoff_ms,
  ...
  return { audit_mode: mode, current, record, leaderboard, rules: { inference: 'descriptive-only', note: '滚动排名不代表下一期优势，需独立前向验证。', min_n: TOP3_MIN_N, min_z: TOP3_MIN_Z, ai_wait_ms: TOP3_AI_WAIT_MS, candidates: CANDIDATES }, history, odds_note: `每注 1，命中 +${pnlOf(true, ARENA_N)}，未中 ${pnlOf(false, ARENA_N)}；保本命中率 52.6%` }
}
