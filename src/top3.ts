...
import { STRATEGIES, META_K, ARENA_N, nextOf, loadPerf, pnlOf, type PerfMap } from './arena'

const r3 = (x: number) => Math.round(x * 1000) / 1000
const CANDIDATES = STRATEGIES.filter(s => !s.control && !s.ai).map(s => s.key)
...
export function rankTop3(perf: PerfMap): Top3Member[] {
  const rows = CANDIDATES.map(k => {
    const r = (perf[k] || []).slice(-META_K)
    const n = r.length, hits = r.reduce((s, x) => s + x.hit, 0), exp = r.reduce((s, x) => s + x.p, 0), varr = r.reduce((s, x) => s + x.p * (1 - x.p), 0)
    const z = n && varr > 0 ? (hits - exp) / Math.sqrt(varr) : 0
    return { key: k, z: r3(z), rate: n ? r3(hits / n) : null, n, hits, w: 0 }
  })
  let top = rows.filter(r => r.n >= TOP3_MIN_N).sort((a, b) => b.z - a.z || b.n - a.n).slice(0, 3)
  if (top.length < 3) { for (const k of ['meta', 'vote', 'quant']) { if (top.length >= 3) break; if (!top.some(t => t.key === k)) top.push(rows.find(r => r.key === k)!) } }
...
export async function top3Round(db: D1Database, source: string, latestExpect: string) {
  ...
  const rounds = (await db.prepare(`SELECT strategy, numbers FROM arena_rounds WHERE source=? AND expect=? AND scored_ms IS NULL`).bind(source, next).all<any>()).results
...
export async function top3Backfill(db: D1Database, source: string, maxPeriods = 100) {
  ...
  for (const m of missing.reverse()) {
    ...
    const fused = fuseTop3(lists); const idx = fused.indexOf(m.actual); const hit = idx >= 0; const ts = Date.now()
...
export async function top3View(db: D1Database, source: string, hist = 12) {
  ...
  const leaderboard = CANDIDATES.map(k => { const r = (perf[k] || []).slice(-META_K); const n = r.length, hits = r.reduce((s, x) => s + x.hit, 0), exp = r.reduce((s, x) => s + x.p, 0), varr = r.reduce((s, x) => s + x.p * (1 - x.p), 0); const d = defOf(k); return { key: k, short: d.short, name: d.name, color: d.color, n, hits, rate: n ? r3(hits / n) : null, z: n && varr > 0 ? r3((hits - exp) / Math.sqrt(varr)) : 0, total: rec(k) } }).sort((a, b) => b.z - a.z)
  // 历史
  ...
  return { current, record, leaderboard, history, odds_note: `每注 1，命中 +${pnlOf(true, ARENA_N)}，未中 ${pnlOf(false, ARENA_N)}；保本命中率 52.6%` }
}
