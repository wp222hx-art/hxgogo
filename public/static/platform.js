(()=>{
 ...
 const builtin=s=>/^qkltj:(600[1-4]|7001)$/.test(s);const valid=builtin(source)||(source==='local:five'&&!['/workspace','/atlas','/simulator','/'].includes(location.pathname)),sel=document.getElementById('hp-source'),status=document.getElementById('hp-status');
 ...
 for(const a of document.querySelectorAll('#hp-sidebar a,#hp-header a')){const u=new URL(a.href);if(valid&&(builtin(source)||!['/workspace','/atlas','/simulator'].includes(u.pathname)))u.searchParams.set('source',source);a.href=u.pathname+u.search}
