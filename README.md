...
### 量化选号器：下一期 Top-N 五位号预选 + 一键复制（`/analysis` 中部 `#pick-section`）
- **数量拖拽自定义**：滑杆 10~2000 注（步进 10）+ 数字输入 + 预设 100/200/300/400/500/800/1000 三者联动，350ms 防抖后重算；标题实时显示「下一期期号 · 共 N 注 · 当前显示」
- **算法（`src/picker.ts`）**：每位 0~9 倾向分布 = 20 机制集成概率 × 单双修正（复用单双 K 线预判 pOdd，权重 0.6）× 大小修正（权重 0.4），再按「分散度」温度 1/1.5/2.2 做 pow(1/temp) 展平；五位联合概率用最大堆 K-best 取 3N 候选，再按组合级信号（总和大小/单双、龙虎、形态先验 [0.01,0.048,0.27,0.672]）重排取 Top-N
- **分层与标签**：核心 10% / 主力 40% / 外围 50%，卡片按层配色；每注显示排名，鼠标点击任一号码可单独复制
- **等价复式方案**：贪心按每位分布挑选子集，使乘积 ≈ N，给出「万[8] 千[0123456789] …」文案与覆盖率，可一键复制
- **量化覆盖率**：Top-N 倾向概率合计 vs 理论基线 N/100000，显示倾向倍数；同时给出每位「种数 / 首数占比」分散度
- **一键复制**：空格分隔 / 逗号分隔 / 每行一个 / 仅核心层 / 复式方案 五种格式，`navigator.clipboard` 失败自动回退 `execCommand('copy')`，成功后显示「已复制到剪贴板」；下方只读文本框可手工选取
- **诚实回测**：最近 20 期用「当期之前的数据」生成 Top-N，统计真实开奖是否落入（命中/期数、实际覆盖率、理论基线、理论期望命中）+ 最近 20 期命中序列；实测 300~500 注命中 0/20（期望 0.06~0.1），页面明确标注「有据可循的随机选号器，非预测」
- **性能**：机制权重仅计算一次并在回测各步复用，bt=20 约 1.4s、bt=0 约 0.5s；结果 60s 缓存并带 `cached:true` / `X-Cache: HIT`，数据版本变更即失效；页面按开奖间隔/3（15~60s）自动刷新
...
| GET | `/api/analysis/pick?source=&count=10-2000&steps=20-150&bt=0-60&wp=&ws=&wc=&temp=0.5-3` | **量化选号器**：下一期 Top-N 五位号（含 rank/tier/p/lift/tags）+ 每位倾向分布 + 组合信号 + 分层统计 + 分散度 + 等价复式方案 + 覆盖率倍数 + 诚实回测 |
