...
### AI 精选档位：前 100 / 150 / 300 / 450 注（策略 key `ai-100` `ai-150` `ai-300` `ai-450`）
- **450 注档**（新增）：AI 排序前 450 注，保本 47.4%，命中 +500 / 未中 −450；比 500 注少压 50 个最弱组合。已由 `ai` 行 rank 回填全部历史（6001 厅 1631 期），/ai 标签栏、逐期记录矩阵、/query 各面板自动出现
...
- **投注档位分析面板**（`GET /api/ai/tier-analysis?source&conf=0.6&round=10`）：对每个档位（100/150/自定义★/300/450/500）给出——
  - **下一期命中概率**：理论 N/1000；全量命中率（Wilson 95% 区间）；近 100 / 近 30；**综合估计** = 全量 50% + 近 100 30% + 近 30 20%；相对保本线的 edge 与判定（有利 / 边际 / 不利，"有利"要求综合估计 ≥ 保本 +2 点且全量不明显低于保本）
  - **长龙机制**：连挂段长度分布、最长、当前连挂；**条件命中率** 挂0→…挂6+→（历史上处于同样连挂长度时下一期真实命中率，高亮当前状态）；**存活率** L1–L6（已挂 L 期后继续挂的实测概率 vs 理论 q）——检验"长龙后更容易中"是否成立
  - **连续进坑**：从现在起再挂 1/2/3/4 期的概率（理论 → 按综合估计）；一轮 10/20 期内至少出现一次 ≥4 连挂的概率（马氏链精确解）
  - **每 10 期倍投回测**（每轮独立、轮末清零）：平注 vs **命中后翻倍**（本期中且下期 AI 置信度 ≥ 阈值 → 下期 ×2，连中续翻最高 ×8，未中回 1）vs **挂后加码 1-2-4**；输出累计、ROI、最大回撤、赢轮数、最差轮、翻倍下注次数与命中率；并展示"命中后下一期实测命中率"及"置信度达阈值时"的子样本。顶部显示下一期 AI 置信度是否达到倍投阈值。阈值 不看/≥50/60/70%、每轮 10/20 期可切
  - **当前结论（1633 期）**：所有档位"命中后翻倍"均不优于平注（命中后下一期命中率 ≈ 无条件命中率，置信度 ≥60% 子样本亦无提升），马丁加码回撤放大 2–3 倍；条件命中率与存活率均贴近理论 → 逐期独立，长龙不会"憋出"命中
- **连挂风险面板**（`GET /api/ai/streaks?source&k=4&n=`）：AI 各投注档位（100/150/自定义★/300/500）出现「连续 ≥K 期不命中」的概率。理论列：单期不中 q = 1 − N/1000，任意 K 连挂 = q^K，段达 K（已挂 1 期后延续到 ≥K）= q^(K−1)，每 100 期期望段数 = 100·(1−q)·q^K；实测列：按时间切分连挂段，统计 ≥K 段占比、处于 ≥K 连挂中的期占比、每 100 期段数、最长/当前连挂、长度分布 1·2·3·4·5·6+。绿 = 实测优于理论，红 = 差于理论；阈值 ≥3/4/5/6、样本 近 200/500/全部 可切。当前（1060 期，K=4）：100 注 段达 4 实测 71.7% vs 理论 72.9%；500 注 13.8% vs 12.5%
