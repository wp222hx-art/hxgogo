...
> **v0.5.3 七套外观与状态高亮**：右上角即时切换股市、绿金竞技、赛博朋克、游戏、科技金融、加密交易所和暖米白主题，选择与命中高亮、收益着色及本机偏好记忆。
...
### AI 策略集 · 每个注数独立生成 + 二级精准（核心改造）⭐⭐
**不再是"500 注排序往下截"**。每期 AI 推理输出同一份 `pos_weights / strategy_blend / boost / avoid` 后，用**不同配方**对全部 1000 个号码重新打分并各自取 top-N（模块 `src/ai_sets.ts` `tierScoreVector` / `lensFor`）：

| 策略 | 注数 | 配方（决定排序的信号配比） | 保本 |
|---|---|---|---|
| `ai` 主推 | 500 | AI 定位^0.65 × 量化共识^0.35，boost×1.6 / avoid×0.4 | 52.6% |
| `ai-100` 双确认 | 100 | **只在核心号内**（万位前 4 × 千/百位前 6 = 144 个），AI 与量化几何平均（0.5/0.5），boost×2.2 / avoid×0.2 | 10.5% |
| `ai-150` AI 主见 | 150 | 几乎纯 AI 定位（0.9/0.1），boost×2.0 | 15.8% |
| `ai-300` 均衡 | 300 | AI 与量化对半（0.5/0.5），boost×1.6 | 31.6% |
| `ai-450` 量化底盘 | 450 | 量化主导（0.35/0.65），AI 微调，avoid 直接剔除 | 47.4% |
| `ai-custom-N` | 自定义 | 按 N 落入上述配方区间 | N/950 |
| `ai-sharp-100` ⚡二级精准 | 100 | **对全部一级生成做加权共识**：评委 = 主推 + 各注数独立生成 + A–E 五组；号码分 = Σ w×(1−rank/len)，w = 评委滚动 40 期 z（截 [0.3,3]）× 精度因子（注数越少越可信，500 注 0.5）；取前 100 | 10.5% |
| `ai-sharp-200` ⚡二级精准 | 200 | 同上取前 200 | 21.1% |

- **为何要"配方"而非"锐度"**：幂变换不改变排序，只调锐度得到的 top-N 仍是同一前缀。真正让 100 注与 500 注成为不同选择的，是 AI 定位 / 量化共识的配比、核心号限定、boost/avoid 力度。实测同期：100 注与 150 注 ∩=96（非子集），300 ⊄ 450（∩=297），450 注有 28–38 个号码在 500 主推之外
- **统计口径全面切换**：`/api/ai/history` `query` `streaks` `tier-analysis` 与 `aiPick.subsets`、`tierDigest`（喷给模型的学习摘要）全部改为读取**各档位自己的结算行**（`loadTierPeriods` / `tierStat`），不再用"主榜 rank ≤ N"推算；改造前无独立行的期显示为虚框 `—` 并在统计中跳过
- **历史重算** `POST /api/ai/tiers/backfill?source&n≤20`：用当期 AI 真实输出按新配方重新生成各档 + 二级精准（删除旧前缀行后重插并即时结算，严格无前视）；与 `sets/backfill` 交替小步运行
- **页面**：`/ai` 标签栏「主推 500 / ⚡精准 100 / 100 / 150 / ⚡精准 200 / 200★ / 250★ / 300 / 450」，选中任一档显示**该档自己的号码**（与主推重叠数、主推之外数，主推之外的号码青色边框）、一键复制；战绩卡带「独立 / 二级 / 自定义」徽标；逐期矩阵列头 ⚡ 标二级、虚框 = 该期无独立记录；`/query` 同步
- **AI 学习反馈**：`tierDigest` 现在按 `kind = main / independent / custom / sharp` 分别汇报各档真实 edge，提示词明确"小注数是用你的 pos_weights 独立生成的，若 edge 持续为正可更果断"
...
| POST | `/api/ai/tiers/backfill?source=&n=8` | 用历史 AI 输出按新配方重算各注数独立生成 + 二级精准（取代旧前缀行） |
